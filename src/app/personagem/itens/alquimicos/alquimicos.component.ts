...
import { ItemEsotericoSB as ItemAlquimicoSB } from '@app/model/supamodel/item.esoterico.sb';
...
import { ItemAlquimicoServiceSupabase } from '@app/service/supaservice/item.alquimico.service.supabase';
...
export class AlquimicosComponent implements OnInit{
  ...
  numero_registros = 0;
  numero_registros_preparados = 0;
  numero_registros_catalizadores = 0;
  numero_registros_venenos = 0;
  filtro_traco: string = '';

  dataSource = new MatTableDataSource<Item>();
  dataSourceP = new MatTableDataSource<Item>();
  dataSourceC = new MatTableDataSource<Item>();
  dataSourceV = new MatTableDataSource<Item>();
  ...
  constructor(
    ...
    private readonly itemAlquimicoServiceSupabase: ItemAlquimicoServiceSupabase,
  ...
  isOddC(element: any): boolean {
    const index = this.dataSourceC.data.indexOf(element);
    return index % 2 !== 0; // Vai adicionar a classe zebra APENAS nas linhas ímpares
  }

  isOddP(element: any): boolean {
    const index = this.dataSourceP.data.indexOf(element);
    return index % 2 !== 0; // Vai adicionar a classe zebra APENAS nas linhas ímpares
  }

  isOddV(element: any): boolean {
    const index = this.dataSourceV.data.indexOf(element);
    return index % 2 !== 0; // Vai adicionar a classe zebra APENAS nas linhas ímpares
  }
  ...
  seleciona(objeto: Item) {
    ...
    setTimeout(() => {
      if (this.itemSB) {
        ...
        this.form.get('idAlquimico')?.setValue(this.itemSB.id);
      ...
      this.selecionaAlquimico(objeto);
  ...
  async selecionaAlquimico(objeto: Item) {
    if (this.form.get('idAlquimico')?.value) {
      try {
        this.itemAlquimico = await this.itemAlquimicoServiceSupabase.consultarPorId(
          this.form.get('idAlquimico')?.value
        );
      } catch (err) {
        console.error('Erro ao carregar Item Arma', err);
      }
    ...
    if (this.itemAlquimico) {
      this.form.get('espaco')?.setValue(this.itemAlquimico.espaco);
    } else {
  ...
  async salvar() {
    ...
    try {
      ...
      if (this.form.get('idAlquimico')?.value) {
        await this.itemAlquimicoServiceSupabase.atualizar(id, this.itemAlquimico);
      } else {
        this.itemAlquimico.id = id;
        this.itemAlquimico.id_item_manutencao = id;
        await this.itemAlquimicoServiceSupabase.inserir(this.itemAlquimico);
      }
  ...
  consultarTodos(filtro: string) {
    this.service.listar(filtro).subscribe({
      ...
      complete: () => {
        ...
        this.preparados = naoMagico.filter((p) =>
          p.chave.includes(Chave.TIPO_ITEM_ALQUIMICO_PREPARADO)
        );
        this.venenos = naoMagico.filter((p) =>
          p.chave.includes(Chave.TIPO_ITEM_ALQUIMICO_VENENO)
        );
        this.catalizadores = naoMagico.filter((p) =>
          p.chave.includes(Chave.TIPO_ITEM_ALQUIMICO_CATALIZADOR)
        );

        this.dataSourceP = new MatTableDataSource(this.preparados);
        this.dataSourceP.paginator = this.paginator;
        this.numero_registros_preparados = this.preparados.length;

        this.dataSourceV = new MatTableDataSource(this.venenos);
        this.dataSourceV.paginator = this.paginator;
        this.numero_registros_venenos = this.venenos.length;

        this.dataSourceC = new MatTableDataSource(this.catalizadores);
        this.dataSourceC.paginator = this.paginator;
        this.numero_registros_catalizadores = this.catalizadores.length;        
