...
import { ItemEquipamentoAventuraSB } from '@app/model/supamodel/item.equipamento.aventura.sb';
...
import { ItemEquipamentoAventuraDao } from '@app/dao/item/item.equipamento.aventura.dao'; 
...
export class EquipamentoAventuraComponent implements OnInit{
  ...
  itemEquipamentoAventura?: ItemEquipamentoAventuraSB;
  ...
  async carregarTabelasDominio() {
    try {
      ...
      this.regraServiceSB.carregarCombo(47).then((regras: Regra[]) => {
        regras.forEach((r) => {
          this.regras.push(r);
        });
  ...
  limparSelecao() {
    ...
    this.itemEquipamentoAventura = undefined;
  ...
  seleciona(objeto: Item) {
    ...
    setTimeout(() => {
      if (this.itemSB) {
        ...
        this.form.get('idEquipamentoAventura')?.setValue(this.itemSB.id);
      ...
      this.selecionaEquipamentoAventura(objeto);
  ...
  async selecionaEquipamentoAventura(objeto: Item) {
    if (this.form.get('idEquipamentoAventura')?.value) {
      try {
        this.itemEquipamentoAventura = await this.itemEquipamentoAventuraSB.consultarPorId(
          this.form.get('idEquipamentoAventura')?.value
        );
      } catch (err) {
        console.error('Erro ao carregar Item Arma', err);
      }
    ...
    if (this.itemEquipamentoAventura) {
      this.form.get('espaco')?.setValue(this.itemEquipamentoAventura.espaco);
    } else {
  ...
  async salvar() {
    ...
    try {
      ...
      if (this.form.get('idEquipamentoAventura')?.value) {
        await this.itemEquipamentoAventuraSB.atualizar(id, this.itemEquipamentoAventura);
      } else {
        this.itemEquipamentoAventura.id = id;
        this.itemEquipamentoAventura.id_item_manutencao = id;
        await this.itemEquipamentoAventuraSB.inserir(this.itemEquipamentoAventura);
      }
